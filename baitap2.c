...
void parse_command(char *input, char **args, int *argc, int *has_pipe, char **args_pipe) {
    *argc = 0;
    *has_pipe = 0;
    ...
    while (should_run) {
        ...
        if (has_pipe) {
            ...
            if (child_pid == 0) { // Tiến trình con
                ...
                waitpid(child_pid, NULL, 0); // Đợi tiến trình con hoàn thành
                child_pid = -1; // Đặt lại child_pid sau khi tiến trình con kết thúc
            }
        }
    }
    return 0;
